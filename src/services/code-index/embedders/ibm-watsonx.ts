...
/**
 * IBM watsonx embedder implementation
 *
 */
export class IbmWatsonxEmbedder implements IEmbedder {
	...
	constructor(
		...
		let options: UserOptions = {
			version: IbmWatsonxEmbedder.WATSONX_VERSION,
		}
		...
		this.modelId = modelId || IbmWatsonxEmbedder.DEFAULT_MODEL

		if (platform === "ibmCloud") {
			if (!apiKey) {
				throw new Error("API key is required in IBM watsonx embedded provider for IBM Cloud")
			}

			if (!projectId) {
				throw new Error("Project ID is required in IBM watsonx embedded provider for IBM Cloud")
			}

			if (!baseUrl) {
				throw new Error("Base URL is required in IBM watsonx embedded provider for IBM Cloud Pak for Data")
			}
			...
			if (!projectId) {
				throw new Error("Project ID is required in IBM watsonx embedded provider for IBM Cloud Pak for Data")
			}
			if (!username) {
				throw new Error("Username is required in IBM watsonx embedded provider for IBM Cloud Pak for Data")
			}
			if (!authType) {
				throw new Error(
					"Auth Type selection is required in IBM watsonx embedded provider for IBM Cloud Pak for Data",
				)
			}
			if (authType === "apiKey" && !apiKey) {
				throw new Error("API key is required in IBM watsonx embedded provider for IBM Cloud Pak for Data")
			}
			if (authType === "password" && !password) {
				throw new Error("Password is required in IBM watsonx embedded provider for IBM Cloud Pak for Data")
			}
			options.serviceUrl = baseUrl
			if (username) {
				if (password) {
		...
		this.watsonxClient = WatsonXAI.newInstance(options)
	}
